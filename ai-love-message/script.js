...
function applyTheme(theme) {
    ...
    switch (theme) {
        case 'Night Sky': document.body.classList.add('theme-night-sky'); break;
        case 'Cute Sticker': document.body.classList.add('theme-cute-sticker'); break;
        case 'Elegant Letter': document.body.classList.add('theme-elegant-letter'); break;
    }
}
...
function validateForm() {
    let isValid = true;
    if (!recipientInput.value.trim()) { showError('recipient'); isValid = false; }
    if (!relationshipSelect.value) { showError('relationship'); isValid = false; }
    if (!moodSelect.value) { showError('mood'); isValid = false; }
    return isValid;
}

function showError(field) {
    const errorEl = document.getElementById(`error${field.charAt(0).toUpperCase() + field.slice(1)}`);
    const inputEl = field === 'recipient' ? recipientInput : field === 'relationship' ? relationshipSelect : moodSelect;
    ...
    try {
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                if (data.title && data.message) {
                    return {
                        title: data.title, opening: data.opening || 'Ada pesan untukmu...',
                        message: data.message, closing: data.closing || 'Dengan cinta.',
                        buttonText: data.buttonText || '💝 Buka Kejutan'
                    };
                }
            }
        } catch (e2) { console.error('Failed to parse Gemini JSON:', e2); }
    }
    return null;
}
...
function getFallbackMessage(recipientName) {
    const idx = Math.floor(Math.random() * fallbackMessages.length);
    const template = { ...fallbackMessages[idx] };
...
function initActionButtons() {
    ...
    btnLink.addEventListener('click', () => {
        if (!currentResult) return;
        const shareData = {
            t: currentResult.title, o: currentResult.opening,
            m: currentResult.message, c: currentResult.closing,
            b: currentResult.buttonText, th: currentResult.formData?.theme || 'Pink Love'
        };
    ...
    btnWhatsApp.addEventListener('click', () => {
        if (!currentResult) return;
        const text = `💌 ${currentResult.title}\n\n${currentResult.opening}\n\n${currentResult.message}\n\n${currentResult.closing}`;
        let shareUrl = shareLink.value ? `\n\n🔗 Buka pesan lengkap: ${shareLink.value}` : '';
        window.open(`https://wa.me/?text=${encodeURIComponent(text + shareUrl)}`, '_blank');
    });

    btnReset.addEventListener('click', () => { resetToForm(); });
}
...
function saveResult(result) {
    try { localStorage.setItem('lastLoveMessage', JSON.stringify(result)); }
...
function loadLastResult() {
    ...
    try {
        ...
        if (saved) currentResult = JSON.parse(saved);
...
function triggerCelebration() {
    ...
    for (let i = 0; i < 20; i++) {
        setTimeout(() => {
            ...
            floatingElements.appendChild(item);
            setTimeout(() => { if (item.parentNode) item.parentNode.removeChild(item); }, 7000);
        }, i * 100);
    }
}
